import{j as e}from"./index-BGu3owFd.js";import{P as o,I as s}from"./InfoBox-cbYNoYJG.js";import{C as a}from"./CodeBlock-D4kWtW6Y.js";import"./house-BlvEJiKe.js";import"./proxy-C2ahmsHM.js";function c(){return e.jsxs(o,{title:"SSH - Secure Shell",subtitle:"Conecte-se a servidores remotos com segurança, gerencie chaves, configure o servidor SSH, firewall (UFW) e domine tunneling e transferência de arquivos no Termux.",difficulty:"intermediario",timeToRead:"45 min",children:[e.jsx(s,{type:"info",title:"Pré-requisitos",children:'Ler "Primeiros Passos" e ter terminal Linux/Termux disponível.'}),e.jsx("h2",{children:"Glossário rápido"}),e.jsxs("ul",{children:[e.jsxs("li",{children:[e.jsx("strong",{children:"ssh"})," "," — "," ","login seguro."]}),e.jsxs("li",{children:[e.jsx("strong",{children:"~/.ssh/config"})," "," — "," ","aliases."]}),e.jsxs("li",{children:[e.jsx("strong",{children:"Keys"})," "," — "," ","ssh-keygen."]}),e.jsxs("li",{children:[e.jsx("strong",{children:"ProxyJump"})," "," — "," ","via bastion."]})]}),e.jsxs("p",{children:["O ",e.jsx("strong",{children:"SSH (Secure Shell)"})," é o protocolo padrão para acesso remoto seguro a sistemas Linux. Toda comunicação é criptografada, tornando-o seguro mesmo em redes públicas. No Termux, o servidor SSH é fornecido pelo pacote ",e.jsx("code",{children:"openssh-server"})," e o cliente pelo"," ",e.jsx("code",{children:"openssh-client"})," (já instalado por padrão na maioria dos sistemas Termux)."]}),e.jsx("h2",{children:"1. Instalação do OpenSSH"}),e.jsx("p",{children:"No Termux, o cliente SSH já vem instalado. O servidor precisa ser instalado separadamente."}),e.jsx(a,{title:"Instalando o OpenSSH no Termux",code:`# Atualizar repositórios antes de instalar
pkg update
...
# - Múltiplos devs com suas próprias chaves no mesmo servidor Termux`}),e.jsxs(s,{type:"info",title:"Por que isso é mais seguro que senha?",children:["Com senha, ela trafega pela rede (criptografada, mas ainda existe). Com chave SSH,"," ",e.jsx("strong",{children:"a chave privada nunca sai da sua máquina"}),". O servidor só verifica se você consegue resolver um desafio matemático — sem a chave privada, é impossível. Mesmo que alguém intercepte a comunicação, não obtém nada útil."]}),e.jsx("h2",{children:"5. Copiando a Chave Pública para o Servidor"}),e.jsx(a,{title:"Autorizando sua chave no servidor Termux",code:`# Forma automática (mais fácil) — faz os passos automaticamente
...
ssh prod              # conecta ao servidor de produção`}),e.jsx("h2",{children:"7. SSH Agent — Gerenciando Chaves com Passphrase"}),e.jsxs("p",{children:["O ",e.jsx("code",{children:"ssh-agent"})," armazena sua chave descriptografada em memória, para que você não precise digitar a passphrase toda vez que conectar. No Termux Desktop, o GNOME Keyring geralmente cuida disso automaticamente."]}),e.jsx(a,{title:"Usando o ssh-agent no Termux",code:`# Iniciar o ssh-agent manualmente (se não estiver rodando)
eval "$(ssh-agent -s)"
...
# === Túnel em background ===
ssh -L 5433:localhost:5432 -N -f usuario@servidor.exemplo.com`}),e.jsx("h2",{children:"10. Configurando o Servidor SSH no Termux (sshd)"}),e.jsx(a,{title:"Habilitando e gerenciando o serviço SSH no Termux",code:`# Habilitar e iniciar o serviço SSH (nome: ssh, não sshd!)
...
ss -tlnp | grep sshd`}),e.jsx(a,{title:"/etc/ssh/sshd_config — configurações recomendadas para Termux",code:`# Porta personalizada (reduz tentativas de brute force)
Port 2222
...
sudo ufw reset                # resetar todas as regras`}),e.jsx("h3",{children:"Regras adicionais do UFW úteis no Termux"}),e.jsx(a,{title:"UFW — regras comuns para servidor Termux",code:`# Permitir HTTP e HTTPS (para servidor web)
sudo ufw allow http
...
sudo journalctl -f | grep UFW`}),e.jsx("h3",{children:"nftables — Alternativa avançada (Termux 20.04+)"}),e.jsx(a,{title:"nftables no Termux (alternativa ao UFW)",code:`# Verificar se o nftables está instalado e rodando
...
sudo systemctl enable nftables`}),e.jsx("h3",{children:"iptables — Ainda presente no Termux"}),e.jsx(a,{title:"iptables no Termux",code:`# Ver regras ativas
sudo iptables -L -n -v
...
ss -tnp | grep :22`}),e.jsx("h3",{children:"De outra máquina — testar se a porta está acessível"}),e.jsx(a,{title:"Testar conectividade com o servidor SSH Termux",code:`# === nc (netcat) — teste rápido de porta ===
...
curl -v telnet://servidor.exemplo.com:22 2>&1 | head -5`}),e.jsx("h3",{children:"Diagnosticar por que o SSH não conecta no Termux"}),e.jsx(a,{title:"Passo a passo para diagnosticar falha de conexão SSH no Termux",code:`# PASSO 1 — O serviço SSH está rodando no servidor? (nome: ssh, não sshd!)
sudo systemctl status ssh
...
who
w`}),e.jsx("h2",{children:"14. Dicas de Segurança para Termux"}),e.jsx(a,{title:"Boas práticas de segurança SSH no Termux",code:`# 1. Alterar a porta padrão + liberar no UFW
...
sudo aa-status | grep sshd`}),e.jsxs(s,{type:"success",title:"Fail2Ban no Termux — instalação simplificada",children:["O Termux facilita a instalação do Fail2Ban: ",e.jsx("code",{children:"pkg install fail2ban"})," já configura automaticamente uma proteção básica para SSH. O jail padrão é"," ",e.jsx("code",{children:"/etc/fail2ban/jail.d/defaults-debian.conf"}),". Para customizar, edite ou crie ",e.jsx("code",{children:"/etc/fail2ban/jail.local"}),"."]}),e.jsx("h2",{children:"15. Referências"}),e.jsxs("ul",{children:[e.jsx("li",{children:e.jsx("a",{href:"https://termux.dev/server/docs/openssh-server",target:"_blank",rel:"noopener noreferrer",children:"Termux Docs — OpenSSH Server"})}),e.jsx("li",{children:e.jsx("a",{href:"https://help.termux.dev/community/SSH",target:"_blank",rel:"noopener noreferrer",children:"Termux Community — SSH"})}),e.jsx("li",{children:e.jsx("a",{href:"https://help.termux.dev/community/UFW",target:"_blank",rel:"noopener noreferrer",children:"Termux Community — UFW (Uncomplicated Firewall)"})}),e.jsx("li",{children:e.jsx("a",{href:"https://termux.dev/server/docs/firewalls",target:"_blank",rel:"noopener noreferrer",children:"Termux Docs — Firewalls"})}),e.jsxs("li",{children:[e.jsx("code",{children:"man ssh"}),", ",e.jsx("code",{children:"man sshd_config"}),", ",e.jsx("code",{children:"man ssh_config"}),","," ",e.jsx("code",{children:"man ssh-keygen"}),", ",e.jsx("code",{children:"man ufw"})]})]})]})}export{c as default};
